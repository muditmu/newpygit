...
def ReadMemory(plc,byte,bit,datatype): #define read memory function
    ...
    if datatype==S7WLBit:
        ...
        return None
def Readinput(plc,byte,bit,datatype): #define read memory function
...
def WriteInput(plc, byte, bit, datatype, value):  # define write memory function
    result = plc.read_area(areas['PE'],  byte, datatype)
    if datatype == S7WLBit:
        set_bool(result, 0, bit, value)
    elif datatype == S7WLByte or datatype == S7WLWord:
        set_int(result, 0, value)
...
#read memory
    ...
    n=Readinput(plc,0,0,S7WLBit)
    print(n)
    input=Readinput(plc,126,0,S7WLWord)
    print('iw126',(input))
   ...
   # print('readreal:',readreal)
    readDword = ReadMemory(plc,50,0,S7WLDWord) #read md50
